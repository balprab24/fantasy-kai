...
| 11.5 | **Spring Boot 3.5 → 4** | Deliberately *not* folded into 5d. 3.5.16 is the head of its line and current; 4.1.1 is the stable major, carrying Spring Framework 7 and Jakarta EE 11. **The first concrete blocker is already known and measured**: Dependabot's PR #11 fails CI in 20s, before a single test runs, because Boot 4's `spring-boot-dependencies` no longer manages `org.testcontainers:postgresql` or `org.testcontainers:junit-jupiter` — both are declared version-less in `pom.xml`, so the POM does not parse. The fix is the Testcontainers BOM or explicit versions, and that is only the error the build reaches *first*. Own phase, own brief, after the perf pass has a stable baseline to compare against. | |
